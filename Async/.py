...
async def consult_tecnicos_horarios(writer, reader):
    ...
    while True:
        ...
        query = ( "SELECT te.id, te.nombre, te.apellido, te.telefono, te.email, z.nombre AS zona, h.semana, h.hora_inicio, h.hora_fin, h.descripcion FROM tecnico te JOIN tecnico_zona tz ON te.id = tz.tecnico_id JOIN zona z ON tz.zona_id = z.id LEFT JOIN turno h ON te.id = h.tecnico_id" )
        crs.execute(query)
